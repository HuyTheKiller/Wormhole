...
extern MY_HIGHP_OR_MEDIUMP vec2 torn_neg;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    vec4 next_pixel = Texel(maskTex, maskuv+vec2(onepixel.x*torn_neg.y*h_dir,onepixel.x*torn_neg.y*v_dir));
    vec4 replace_color = vec4(239., 239., 239., 27.) / 255.; // Replaces white color

    if (uv.y + torn_neg.y == uv.y)
        uv.y = torn_neg.y;
