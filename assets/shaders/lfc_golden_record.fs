...
extern PRECISION vec2 lfc_golden_record;
extern PRECISION vec2 card_size;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    vec2 adjusted_uv = uv * (texture_details.ba / card_size) - vec2(0.5, 0.5);
    adjusted_uv.x = adjusted_uv.x*texture_details.b/texture_details.a;
    

    // polar coordinates
    vec2 base_uv = adjusted_uv;
    ...
    // disc light lines
    float line_count = 2.0;
    float line_size = 0.45;
    float line_offset = -0.1;
    float touch_influence = 0.125;
    float gr_amt = fract((polar_uv.x + line_offset + lfc_golden_record.x*touch_influence) * line_count);
    float gr_lines = smoothstep(0.0, line_size*0.5, (gr_amt)) - smoothstep(line_size*0.5, line_size, (gr_amt));

    // disc lighting
    vec3 hsl = HSL(tex).rgb;
    tex.rgb = RGB(vec4(hsl.r + smoothstep(0.75, 1.0, hsl.b) * 0.05, hsl.g + smoothstep(0.75, 1.0, hsl.b) * 0.08, min(1.0, hsl.b + gr_lines*min(1.0, pow(hsl.b, 2.0))), tex.a)).rgb;
